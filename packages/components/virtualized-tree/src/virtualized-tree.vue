<template>
  <div
    :class="['pro-virtualized-tree', $attrs.class]"
    :style="[$attrs.style as any]"
  >
    <pro-input
      v-if="!$props.hideSearchInput"
      class="pro-virtualized-tree__search-input"
    ...
    <el-tree-v2
      ...
      :class="[
        'pro-virtualized-tree__tree',
        !$props.hideSearchInput ? 'pro-virtualized-tree__tree--has-search' : '',
      ]"
    ...
    >
      <template
        v-for="$virtualizedTreeFilteredSlotName in $virtualizedTreeFilteredSlotNames"
        :key="$virtualizedTreeFilteredSlotName"
        #[$virtualizedTreeFilteredSlotName]="scope"
      >
        <slot
          v-bind="scope"
          :key="$virtualizedTreeFilteredSlotName"
          :name="getSlotName($props, $slots, $virtualizedTreeFilteredSlotName)"
        />
      </template>
      <template #default="scope">
        <div class="pro-virtualized-tree__tree-node">
          <div class="pro-virtualized-tree__tree-node-main">
            <slot
          ...
          <div
            ...
            "
            class="pro-virtualized-tree__tree-node-action"
          >
            <slot
            ...
            />
            <ProDropdown
              class="pro-virtualized-tree__tree-node-action-dropdown"
              v-else-if="
                $internalCurrentNode?.[$defaultProps.value] ===
                scope.data?.[$defaultProps.value]
              "
              @command="
                command => {
    ...
    <pro-button
      ...
      class="pro-virtualized-tree__load-more-button"
...
import { ElLoading, ElMessage, ElTreeV2 } from 'element-plus'
...
/* 属性 */
const $props = withDefaults(defineProps<VirtualizedTreeExtendedProps>(), {
...
/* 注入 */
const $injects = inject<ProVirtualizedTreeInjects>(
  'proVirtualizedTreeInjects',
  {}
)
/* 事件 */
...
const $virtualizedTreeSlotNames = ['default', 'empty']
const $virtualizedTreeFilteredSlotNames = getFilteredSlotNames(
  $props,
  $slots,
  $virtualizedTreeSlotNames
)
/* 实例 */
const $virtualizedTreeRef = ref<VirtualizedTreeInstance>()
/* 辅助参数 */
...
const $onSearchInputSearch = () => {
  ...
  if ($props.requestApi) {
    ...
    $virtualizedTreeRef.value?.filter($internalSearchInputModelValue.value)
  }
}
...
const $defaultCheck = (
  data: TreeNodeData,
  treeCheckedStatusObject: CheckedInfo
) => {
  $internalCheckedNodes.value = treeCheckedStatusObject.checkedNodes
}
const $defaultCurrentChange = (data: TreeNodeData, node: TreeNode) => {
...
const $defaultFilterMethod = (value: string, data: TreeNodeData) => {
  if (!isUndefined($props.requestApi)) {
    return true
  }
  if (!isUndefined($props.filterMethod)) {
    return $props.filterMethod(value, data)
  }
  if (!isUndefined($props.data)) {
    if ($defaultProps.value.label) {
    ...
    if (!isUndefined(value)) {
      ...
      setTimeout(() => {
        const currentCheckedKeys = value.map(
          node => node[$defaultProps.value.value!]
        )
        $virtualizedTreeRef.value?.setCheckedKeys(currentCheckedKeys)
      })
    }
  },
  {
...
watch(
  () => $props.currentNode,
  value => {
    if (!isUndefined(value) && value[$defaultProps.value.value!]) {
      $internalCurrentNode.value = value
      nextTick(() => {
        $virtualizedTreeRef.value?.setCurrentKey(
...
let $internalData = ref<VirtualizedTreeData>([])
watch(
  () => $props.data,
  value => {
    if (!isUndefined(value)) {
...
watch(
  ...
  () => {
    if (Object.keys($internalCurrentNode.value).length) {
      nextTick(() => {
        $virtualizedTreeRef.value?.setCurrentKey(
          $internalCurrentNode.value[$defaultProps.value.value!]
        )
      })
    }
  },
  ...
  value => {
    if (value) {
      $loadingInstance = ElLoading.service({
        target: $virtualizedTreeRef.value?.$el,
      })
    } else {
      if ($loadingInstance) {
    ...
    try {
      ...
      if (res) {
        ...
        if ($props.checkedNodes?.length) {
          setTimeout(() => {
            const currentCheckedKeys = $props.checkedNodes!.map(
              node => node[$defaultProps.value.value!]
            )
            $virtualizedTreeRef.value!.setCheckedKeys(currentCheckedKeys)
          })
